...
> ⭐ **We strongly recommend Claude + GPT-6-Astra (default setup).** It's the most tested and reliable combination. Alternative setups work but may require prompt tuning.

Beyond the default Claude × GPT-6-Astra route, ARIS ships **10 alternative routes (Alt A-I + C.1)** covering Z.ai's GLM, Alibaba's Kimi/Qwen/GLM/MiniMax bundle, ModelScope's free DeepSeek-V3.1, Codex-as-executor with Claude or Gemini reviewers, and Google Antigravity as the executor.
...
|---|----------|----------|:---:|:---:|-------|
| **Default** ⭐ | Claude Opus/Sonnet | GPT-6-Astra (Codex MCP) | Yes | Yes | [Quick Start](#quick-start) |
| **Alt A** | GLM-5 (Z.ai) | GPT-6-Astra (Codex MCP) | No | Yes | [Setup below](#alt-a-glm--gpt) |
...
| **Alt F** | Codex CLI (GPT-6-Astra) | Codex `spawn_agent` (GPT-6-Astra) | No | Yes | [skills-codex/](../skills/skills-codex/) |
| **Alt G** 🆕 | Codex CLI | Claude Code CLI (`claude-review` MCP) | No* | No* | [CODEX_CLAUDE_REVIEW_GUIDE](CODEX_CLAUDE_REVIEW_GUIDE.md) |
| **Alt H** 🆕 | Antigravity (Claude Opus 4.6 / Gemini 3.1 Pro) | GPT-6-Astra (Codex MCP) or any via llm-chat | No | Optional | [ANTIGRAVITY_ADAPTATION](ANTIGRAVITY_ADAPTATION.md) |
...
- **Alt A** — only swap Claude for GLM, keep GPT-6-Astra as reviewer via Codex MCP.
...
- **Alt H** — Antigravity as the executor (Claude Opus 4.6 or Gemini 3.1 Pro), GPT-6-Astra or any `llm-chat` reviewer.
...
Only replace the executor (Claude → GLM via Z.ai), keep GPT-6-Astra as reviewer via Codex MCP. Codex CLI reuses your existing `OPENAI_API_KEY` (from `~/.codex/config.toml` or environment); no extra reviewer-side config.
...
codex setup   # set model to gpt-6-astra
```
...
> ⚠️ **Note:** Alternative models may behave differently from Claude and GPT-6-Astra. You may need to tune prompt templates for best results. The core cross-model architecture remains the same.
