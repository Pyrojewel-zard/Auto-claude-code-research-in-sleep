...
> ⭐ **强烈推荐使用 Claude + GPT-6-Astra（默认组合）。** 这是经过最充分测试、最稳定的组合。替代方案可用但可能需要调整 prompt。

除了默认的 Claude × GPT-6-Astra，ARIS 还内置 **10 条替代路由（方案 A-I + C.1）**，覆盖 Z.ai 的 GLM、阿里百炼的 Kimi/Qwen/GLM/MiniMax 套餐、ModelScope 免费的 DeepSeek-V3.1、Codex 作为执行者搭配 Claude 或 Gemini 审稿、以及 Google Antigravity 作为执行器。
...
|---|--------|--------|:---:|:---:|---------|
| **默认** ⭐ | Claude Opus/Sonnet | GPT-6-Astra（Codex MCP） | 是 | 是 | [快速开始](#quick-start) |
| **方案 A** | GLM-5（Z.ai） | GPT-6-Astra（Codex MCP） | 否 | 是 | [配置见下](#alt-a-glm--gpt) |
...
| **方案 F** | Codex CLI (GPT-6-Astra) | Codex `spawn_agent` (GPT-6-Astra) | 否 | 是 | [skills-codex/](../skills/skills-codex/) |
| **方案 G** 🆕 | Codex CLI | Claude Code CLI（`claude-review` MCP） | 否* | 否* | [CODEX_CLAUDE_REVIEW_GUIDE_CN](CODEX_CLAUDE_REVIEW_GUIDE_CN.md) |
| **方案 H** 🆕 | Antigravity（Claude Opus 4.6 / Gemini 3.1 Pro） | GPT-6-Astra（Codex MCP）或 llm-chat | 否 | 可选 | [ANTIGRAVITY_ADAPTATION_CN](ANTIGRAVITY_ADAPTATION_CN.md) |
...
- **方案 A** —— 只换执行者（Claude → GLM），审稿人保留 GPT-6-Astra via Codex MCP。
...
- **方案 H** —— 用 Antigravity 作为执行器（Claude Opus 4.6 或 Gemini 3.1 Pro），GPT-6-Astra 或任意 `llm-chat` 审稿。
...
只替换执行者（Claude → 通过 Z.ai 切到 GLM），保留 GPT-6-Astra 通过 Codex MCP 审稿。Codex CLI 复用你已有的 `OPENAI_API_KEY`（来自 `~/.codex/config.toml` 或环境变量），审稿端不需要额外配置。
...
> ⚠️ **注意：** 替代模型的行为可能与 Claude 和 GPT-6-Astra 有所不同。你可能需要微调 prompt 模板以获得最佳效果。核心的跨模型架构不变。
