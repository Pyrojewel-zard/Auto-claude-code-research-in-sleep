...
## Start here: the reduced ARIS flow
...
For the daily Codex surface, install the minimal profile:

```bash
bash tools/install_aris_codex.sh /path/to/project --profile pyrojewel-research
```

`/autoresearch-topic` starts from a topic; `/autoresearch-proposal` starts from
an existing application/proposal draft. Both use Zotero semantic search,
split independent branches, and finish each branch before handing it to
`/research-audit`. `/research-write` consumes promoted evidence only.
`/research-audit` runs the branch or frozen-artifact Anti-Autoresearch audit
with a fresh isolated Codex reviewer. The old `/research`, `/write`, and
`/audit` names remain compatibility entries, but are outside the default
profile. Future upstream flow candidates are reviewed by
[`docs/UPSTREAM_FLOW_REVIEW.md`](docs/UPSTREAM_FLOW_REVIEW.md) before adoption.
...
>
> **v0.4.8** (2026-05-17) — Skill helper subsystem rewrite. Bundled helpers extract to `~/.config/aris/cache/<version>/` at startup; every Skill invocation surfaces `helperReport` JSON + 4-layer resolver preamble; `/skills export` copies helpers; new `integration-contract.md` with 6 failure policies; 8 shared helpers (arxiv/deepxiv/exa/S2/openalex/save_trace/verify_papers/verify_paper_audits) bundled; `/research-lit` + `/deepxiv` migrated. Plus 4 bug fixes: gpt-5.5+tools 400 on OpenAI; Custom reviewer reset; missing `signature` field ([#228](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/228)); `--version` Build date hardcoded.
>
> **v0.4.7** (2026-05-16) — DashScope Coding Plan 405 fixed ([#159](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/159)) via `native-tls` switch ([#225](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/225)); `reasoning_content` replay for all reasoning models (OpenAI o1/o3/o4 / DeepSeek-R1 etc.), not just Kimi ([#226](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/226)); 600+ lines dead code cleanup + `rustyline` dep removed + "Claw Code" → "ARIS-Code" rebrand.
>
> **v0.4.6** (2026-05-14) — 🚨 Two long-standing silent bugs fixed: `PermissionMode::Prompt` silently allowed every tool (derived-`Ord` bug); system prompt hardcoded `current_date = "2026-03-31"` made models reject post-cutoff data as future/prompt-injection. Plus Custom OpenAI-compatible provider (`/setup` option 11) with dynamic `/models` discovery ([@Anduin9527](https://github.com/Anduin9527) [#221](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/221) + [#222](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/222)).
>
> **v0.4.5** (2026-05-13) — First-class reasoning-model support: thinking content blocks end-to-end (fixes [#161](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/161)) + `reasoning_effort='xhigh'` for GPT-5.5 / o1 / o3 / o4 / DeepSeek-thinking. DeepSeek V4 Pro + Xiaomi MiMo + Qwen 3.6 + Doubao in `/setup` (options 7-10). Object-style hooks parser. Default model bumped to Claude Opus 4.7 + GPT-5.5. REPL input hardening (multi-line wrap / Cmd+V paste / CJK boundary). GitHub Actions CI. Credits: [@GO-player-hhy](https://github.com/GO-player-hhy) ([#186](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/186)), [@Jxy-yxJ](https://github.com/Jxy-yxJ) ([#171](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/171)), [@GetIT-Sunday](https://github.com/GetIT-Sunday) ([#216](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/216) partial).
>
> </details>
>
> <details><summary>Older versions</summary>
>
> **v0.4.4** (2026-04-20) — **Setup UX + reviewer routing fixes** (resolves [#158](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/158), [#162](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/162)) | `/setup` no longer forces Bearer for Anthropic + custom URL | Provider-aware proxy URL hints | Stale state no longer leaks across provider switches | LlmReview smart fallback
>
...
- **2026-05-26** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🤝 **Community PR wave — 5 merges this week**. [`/wiki-enrich`](skills/wiki-enrich/SKILL.md) ([#247](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/247) by [@hungchun0201](https://github.com/hungchun0201)) fills paper TODOs `ingest_paper` leaves as scaffolds — Karpathy LLM-wiki principle, fetch chain alphaxiv→deepxiv→arXiv. [Mirror drift checker + CI](tools/check_skills_inventory.py) ([#241](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/241) by [@VeraPyuyi](https://github.com/VeraPyuyi)) keeps main↔mirror in sync. `/research-pipeline` Stage 2/3 unified into `/experiment-bridge` delegation ([#243](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/243) by [@ZBigFish](https://github.com/ZBigFish)) — old inline was a strict subset of the bridge. Windows PowerShell installer parity with reparse-chain inside-repo guard + `-FromOld` legacy migration + Windows CI matrix ([#242](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/242) by [@VeraPyuyi](https://github.com/VeraPyuyi)). Plus [`manual-review` MCP](mcp-servers/manual-review/README.md) ([#246](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/246) by [@ZBigFish](https://github.com/ZBigFish)) — **third reviewer backend `— reviewer: manual`** for zero-cost cross-model review (paste prompt to any non-Claude model: DeepSeek / Kimi / ChatGPT / Gemini / local llama); cross-model invariant guarded by bilingual UI banner + per-session token auth + fail-closed when MCP unavailable.

- **2026-05-17** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🐙 **[GitHub Copilot CLI adaptation](docs/COPILOT_CLI_ADAPTATION.md)** — native `SKILL.md` + MCP support, no skill mirror needed. Installer (`install_aris_copilot.sh`) + smart-updater + 13-test suite. Community contribution by [@EarendelH](https://github.com/EarendelH) ([#229](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/229), closes [#214](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/214) / [#227](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/227) / [#203](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/203)).
- **2026-05-17** — ![FIX](https://img.shields.io/badge/FIX-orange?style=flat-square) 🛠 **Tools-stability roadmap (Phase 1+2+3) complete** (closes [#176](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/176) / [#177](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/177) / [#178](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/178)). Community reported that helper scripts weren't propagating into user projects after `install_aris.sh`. **Phase 1** — every SKILL.md caller of the 10 canonical helpers now resolves via the strict-safe 3-layer chain `.aris/tools/` → `tools/` → `$ARIS_REPO/tools/` documented in [`integration-contract.md`](skills/shared-references/integration-contract.md) §2 (which also defines 5 failure policies A/B/C/D1/D2/E). **Phase 2** — new [advisory CI lint](.github/workflows/lint-skills-helpers.yml) catches hardcoded `python3 tools/foo.py` patterns in PR-modified SKILL.md (advisory only, never fails CI). **Phase 3** — three single-owner helpers (`figure-spec`, `paper-illustration-image2`, `experiment-queue`) moved into their SKILL's `scripts/` subdirectory; owner SKILLs use Layer 0 `${CLAUDE_SKILL_DIR}/scripts/` ahead of the canonical chain; legacy `tools/` paths retained as `os.execv` Python forwarding shims. **⚠️ Existing users**: no action needed — legacy `tools/` entries are now shims. If you haven't run `install_aris.sh` since 2026-04-30, one idempotent rerun catches everything up.
- **2026-05-14** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🩹 **`/paper-plan` + `/paper-write` learn `GAP_REPORT.md` + `<!-- DATA_NEEDED -->` discipline** ([#217](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/217)). When `— style-ref:` is set and the user's project has structural assets (`figures/`, `results/`, `NARRATIVE_REPORT.md`, etc.), `/paper-plan` emits a **Gap Report** mapping the exemplar's section topology + density (from `style_profile.md`) against your actual assets — surfacing slots you have **no evidence to fill** (e.g., "exemplar has 3×4 ablation table, you have no ablation data"). Then `/paper-write` writes `<!-- DATA_NEEDED: <Slot ID> — <description> -->` HTML comments **instead of fabricating content** at missing slots — invisible in the compiled PDF, `grep`-friendly for human triage / `/experiment-bridge` follow-up. Narrow carve-out from the "no placeholders" rule, scoped to GAP_REPORT-listed slots only. Original idea by [@zhangpelf](https://github.com/zhangpelf).
...
- **2026-04-30** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🪞 **Codex skill mirror rebuilt + dedicated install/update chain** ([#179](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/179), community contribution by [@No-518](https://github.com/No-518)). `skills/skills-codex/` now mirrors all 67 mainline skills; replaces `mcp__codex__codex` reviewer path with Codex-native `spawn_agent` + `send_input`. New [`tools/install_aris_codex.sh`](tools/install_aris_codex.sh) + [`tools/smart_update_codex.sh`](tools/smart_update_codex.sh) handle project-local symlinks with manifest tracking. Anti-drift: `tests/test_codex_skill_mirror.py` + `tests/test_codex_install_update.py` (26 failure paths). Open discussion in [#184](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/184)
- **2026-04-24** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🎨 **[`/paper-illustration-image2`](skills/paper-illustration-image2/SKILL.md)** — Codex-native image generation as Phase 2b illustration backend ([#166](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/pull/166), community contribution by [@kbr19-thu](https://github.com/kbr19-thu) 清华). Uses ChatGPT Plus/Pro quota via local [Codex app-server MCP bridge](mcp-servers/codex-image2/) — **no `GEMINI_API_KEY` required**. Triggered by `/paper-writing — illustration: codex-image2`; default stays `figurespec` (**zero behavior change**). Async-only API, sandboxed writes to `figures/ai_generated/`, [integration-contract](skills/shared-references/integration-contract.md)-compliant helper. Marked **experimental** (Codex debug app-server is unstable upstream)
...
- **2026-04-10** — 🛡️ **[`/experiment-audit`](skills/experiment-audit/SKILL.md)** — cross-model experiment integrity verification. GPT-5.4 reads your eval scripts and results directly, checks for fake ground truth, self-normalized scores, phantom results, and scope inflation ([#131](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/131), [#57](https://github.com/Pyrojewel-zard/Auto-claude-code-research-in-sleep/issues/57)). Advisory — warns loudly, never blocks. `/result-to-claim` auto-reads audit if present. New [experiment-integrity.md](skills/shared-references/experiment-integrity.md) shared reference. **The executor must never judge its own integrity.**
...
# (don't need all 89? --list-groups / --groups X,Y / --skills X / --profile pyrojewel-research — see "Selective install" below)
...
> Don't need all 89 skills? See [Selective install](#install-skills) below for group, skill, or profile-level picks.
...
ARIS ships **89 composable skills** across the whole research lifecycle. The reduced default surface is `/autoresearch-topic` or `/autoresearch-proposal` → `/research-audit` → `/research-write`: Zotero semantic search supplies the literature evidence, promoted branches constrain the draft, and a vendored Anti-Autoresearch engine plus a fresh isolated Codex review audits the frozen output. The remaining skills stay available for legacy and domain-specific workflows.
...
- 📊 **89 composable skills** — the default reduced profile is `pyrojewel-research` (`/autoresearch-topic`, `/autoresearch-proposal`, `/research-write`, `/research-audit`); legacy and domain-specific skills remain available in the [full catalog →](docs/SKILLS_CATALOG.md)
...
ARIS ships **89+ skills** across literature, ideation, experiments, audit, writing, talks, patents, and meta-utilities — the full catalog (role / category / requirements per skill) lives in **[`docs/SKILLS_CATALOG.md`](docs/SKILLS_CATALOG.md)** to keep this README scannable.
...
→ **[Browse all 89 skills by category in the full catalog →](docs/SKILLS_CATALOG.md)**
...
![GitHub stars](https://img.shields.io/github/stars/Pyrojewel-zard/Auto-claude-code-research-in-sleep?style=social)

<a href="https://www.star-history.com/?repos=wanshuiyin%2FAuto-claude-code-research-in-sleep&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=Pyrojewel-zard/Auto-claude-code-research-in-sleep&type=date&theme=dark&legend=top-left&sealed_token=k7GiP-sALgHwi5DeZRCZufnpu4KesUSoNuhjn6qeKlPZ03GyC-EvHB7yFbWOfXmlV2oXduMnFx0zW5-h7m_bKocO3KlfXKrVAAhJQ-au67CkRUp-m3KrRw" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=Pyrojewel-zard/Auto-claude-code-research-in-sleep&type=date&legend=top-left&sealed_token=k7GiP-sALgHwi5DeZRCZufnpu4KesUSoNuhjn6qeKlPZ03GyC-EvHB7yFbWOfXmlV2oXduMnFx0zW5-h7m_bKocO3KlfXKrVAAhJQ-au67CkRUp-m3KrRw" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=Pyrojewel-zard/Auto-claude-code-research-in-sleep&type=date&legend=top-left&sealed_token=k7GiP-sALgHwi5DeZRCZufnpu4KesUSoNuhjn6qeKlPZ03GyC-EvHB7yFbWOfXmlV2oXduMnFx0zW5-h7m_bKocO3KlfXKrVAAhJQ-au67CkRUp-m3KrRw" />
 </picture>
</a>
