...
the `/research-write` handoff. Do not collapse the draft into one topic query.
...
`query pack:` handoff is consumed by `/autoresearch-proposal`, which performs one
...
After branch research, audit, and promotion, invoke `/research-write` with only
