...
TAXONOMY_VERSION   = 0.6       (references/hack-pattern-taxonomy.md; 51 integrity patterns A–I + 13 AIS impressions + 2 ADV advisory; stamped into every report)
...
expects; the `Owns` column is the taxonomy-v0.6 patterns that skill may emit:

| Skill | Run when | Writes | Owns (pattern_ids, taxonomy v0.6) |
|-------|----------|--------|-----------------------------------|
...
`pattern_id` present MUST be in `references/hack-pattern-taxonomy.md` (v0.6); never patch
findings by hand.
...
python3 "$ROOT/tools/adjudicate_findings.py" \
    ...
    --paper-id "$PAPER_ID" --observability-level "$L" --taxonomy-version 0.6 \
...
- **`pattern_id` ∈ taxonomy v0.6 only**; the taxonomy is a post-hoc mapping layer, not
