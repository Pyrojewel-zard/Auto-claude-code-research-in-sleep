...
EVIDENCE_BRANCH = os.path.join(HERE, "fixtures", "evidence_branch")
EXPECT = os.path.join(HERE, "expected_findings")
FIXED_TS = "2026-01-01T00:00:00Z"
CURRENT_TAXONOMY_VERSION = "0.6"
...
def expected_fixture_names():
    return {name[:-5] for name in os.listdir(EXPECT) if name.endswith(".json")}
...
def audit_evidence_branch():
    claims = os.path.join(EVIDENCE_BRANCH, "claims.json")
    coverage = os.path.join(EVIDENCE_BRANCH, "query_coverage.json")
    findings = os.path.join(RUN_DIR, "evidence_branch.findings.json")
    report = os.path.join(RUN_DIR, "evidence_branch.report.json")
    md = os.path.join(RUN_DIR, "evidence_branch.REPORT.md")
    run([os.path.join(TOOLS, "check_evidence_coverage.py"),
         "--ledger", claims, "--coverage", coverage, "--out", findings])
    run([os.path.join(TOOLS, "adjudicate_findings.py"), "--findings", findings,
         "--ledger", claims, "--paper-id", "evidence_branch",
         "--observability-level", "1", "--generated-at", FIXED_TS,
         "--out", report, "--md", md])
    with open(report, encoding="utf-8") as fh:
        return json.load(fh)
...
def evaluate(name, report):
    ...
    severity_mismatches = []
    for pattern_id, expected_severity in exp.get("required_severities", {}).items():
        actual_severities = sorted({f.get("_severity_final") for f in above_info
                                    if f.get("pattern_id") == pattern_id})
        if expected_severity not in actual_severities:
            severity_mismatches.append(
                f"{pattern_id} expected {expected_severity}, got "
                f"{','.join(actual_severities) or 'none'}"
            )
    ...
    taxonomy_mismatch = report.get("taxonomy_version") != CURRENT_TAXONOMY_VERSION
    verr = None
    ...
    ok = not fn and not extra and not fp_clean and not verr and not unanchored \
        and not ais_fn and not ais_leak and not severity_mismatches and not taxonomy_mismatch
    return {
...
def main():
    ...
    fixtures.append(("evidence_branch", None))
    missing_fixtures = sorted(expected_fixture_names() - {name for name, _ in fixtures})
    if missing_fixtures:
        print("missing eval fixture(s) for expected findings: "
              + ", ".join(missing_fixtures), file=sys.stderr)
        return 1
    results = [evaluate(
        name,
        audit_evidence_branch() if name == "evidence_branch" else audit(name, tex)
    ) for name, tex in fixtures]
    ...
    for r in results:
        ...
        if r.get("severity_mismatches"):
            detail.append("SEVERITY=" + "; ".join(r["severity_mismatches"]))
        if r.get("taxonomy_mismatch"):
            detail.append(f"TAXONOMY={r.get('taxonomy_version', 'missing')}"
                          f" expected {CURRENT_TAXONOMY_VERSION}")
