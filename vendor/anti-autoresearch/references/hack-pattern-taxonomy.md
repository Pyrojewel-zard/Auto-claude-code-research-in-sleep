...
```
taxonomy_version: 0.6
last_reviewed: 2026-09-08
patterns: 51 integrity (families A–I) + 13 AI-style impressions (AIS · zero verdict weight) + 2 advisory (zero verdict weight)
...
## I. Research evidence coverage (L0 stated coverage)

> These patterns audit the recorded evidence relationship for a research branch.
> They inspect the claims ledger and query-coverage record only; they do not
> retrieve sources or turn a query outcome into a verdict by themselves. A
> `NO_HIT` query remains a recorded no-hit state and is never treated as support.

### HP-EVIDENCE-UNSUPPORTED — a load-bearing research claim has no verified support
- **level:** L0 (the absence of a verified support record is decidable from the claims and coverage artifacts).
- **signals:** a research claim marked `load_bearing: true` has no searched query with a recorded supporting evidence item and no verified claim-bearing support basis. A `NO_HIT` query is preserved as no supporting hit.
- **fp_cases:** a claim is intentionally a hypothesis; the supporting artifact is recorded in a separate, linked branch; an evidence item is present but its query was marked `UNVERIFIED` (route to HP-EVIDENCE-UNVERIFIED, not this pattern).
- **severity_rule:** critical for a load-bearing claim; minor for a non-load-bearing claim. The finding identifies a coverage gap, never author intent.
- **min_evidence:** the verbatim claim span + its claim id and evidence anchor + the selected query statuses and evidence-item keys.
- **example:** `"The proposed mechanism improves robustness."` is load-bearing, but its only query is `NO_HIT` and records no evidence item.

### HP-EVIDENCE-RELEVANCE-AS-SUPPORT — retrieval relevance is used as if it verified a claim
- **level:** L0 (the declared support basis is recorded in the claims ledger).
- **signals:** `support_basis: relevance_score` is used for a research claim, treating a retrieval ranking/relevance value as the evidence that the claim is true.
- **fp_cases:** relevance is recorded only as a triage field and the claim separately cites a quoted span, annotation, or metadata; a reviewer is merely ranking candidate sources.
- **severity_rule:** critical when the relevance score is the declared support basis; replace it with the claim-bearing evidence or mark the claim unverified. This is a method discrepancy, not an accusation.
- **min_evidence:** the verbatim claim span + the recorded `support_basis` + the associated evidence-item/query record.
- **example:** a claim is marked `support_basis: relevance_score` solely because Zotero returned a high-ranked item.

### HP-EVIDENCE-UNVERIFIED — recorded support has not reached a verified terminal state
- **level:** L0 (the claim and query status are recorded in the coverage artifacts).
- **signals:** a claim or its selected query is `UNVERIFIED`, `VERIFY_PENDING`, `UNSEARCHABLE`, or `ERROR`, even when an evidence item key is present.
- **fp_cases:** a clearly labeled preliminary search; an intentionally unresolved hypothesis; a query whose external verification is documented elsewhere and linked by stable id.
- **severity_rule:** major for support that remains unverifiable; never critical from the status alone. Ask for the missing verification rather than treating a provisional record as false.
- **min_evidence:** the verbatim claim span + claim verification status or query terminal state + the associated evidence-item key(s).
- **example:** a query records `UNVERIFIED` with item `Z1`; the item is not promoted to verified support.

### HP-EVIDENCE-PROVENANCE-GAP — an evidence item lacks an artifact hash
- **level:** L0 (the missing provenance is decidable from the coverage record).
- **signals:** a query names an evidence item key but `artifact_hashes` has no non-empty hash for that key.
- **fp_cases:** a deliberately ephemeral external reference whose stable provenance is recorded in an allowed source registry; a query with no evidence item keys.
- **severity_rule:** minor: the item may be relevant, but the audit cannot bind it to an immutable artifact. Do not escalate the gap to a claim about the source's truth.
- **min_evidence:** the verbatim claim span + the missing evidence-item key + the coverage provenance map.
- **example:** query `Q1` lists `Z1`, but `artifact_hashes` contains no `Z1` entry.

### HP-EVIDENCE-NO-COUNTERSEARCH — the branch records no opposing query
- **level:** L0 (the branch-level counter-query declaration is recorded in the coverage artifact).
- **signals:** `counter_query_run: false` for a branch containing a research claim, so the recorded support has not been tested against an opposing search direction.
- **fp_cases:** a documented unsearchable question; a bounded branch where a counter-query is impossible and the limitation is explicit; a counter-query recorded in a linked coverage branch.
- **severity_rule:** minor: a missing robustness check, not evidence that the claim is wrong.
- **min_evidence:** the verbatim claim span + the branch coverage record declaring that no counter-query ran.
- **example:** the branch records only a supporting Zotero query and `counter_query_run: false`.
