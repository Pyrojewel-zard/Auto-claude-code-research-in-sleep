...
- **TAXONOMY_VERSION = `0.6`** — the ledger is **taxonomy-agnostic** (it tags no `pattern_id`); patterns are applied *post-hoc* by auditors (`references/hack-pattern-taxonomy.md`, now 51 integrity patterns A–I + 13 AIS + 2 advisory). Never tag a claim with a `pattern_id` here. v0.5 migrated the pure-style patterns to the zero-weight AIS track; the ledger stays untagged, but its **enrichment** (Step 3) now surfaces the *anchors* those auditors quote.
...
python3 "$ROOT/tools/adjudicate_findings.py" --findings *.findings.json \
    ...
    --observability-level "$L" --taxonomy-version 0.6 --out report.json --md REPORT.md
```
