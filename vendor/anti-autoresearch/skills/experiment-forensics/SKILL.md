...
- **PATTERNS_OWNED** (must match `references/hack-pattern-taxonomy.md`,
  `taxonomy_version 0.6`): `HP-FAKE-GT`, `HP-SELF-NORM`, `HP-PHANTOM-RESULT`,
...
python3 "$ROOT/tools/adjudicate_findings.py" --findings "$TARGET"/*.findings.json \
    ...
    --taxonomy-version 0.6 --out "$TARGET/report.json" --md "$TARGET/REPORT.md"
```
...
- **`pattern_id` ∈ taxonomy v0.6 only** (`PATTERNS_OWNED`); the taxonomy is a
