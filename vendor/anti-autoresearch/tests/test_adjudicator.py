...
def test_zero_weight_never_moves_overall_verdict():
    ...
    args = types.SimpleNamespace(observability_level=2, limitation=None,
                                 taxonomy_version="0.6", paper_id="t", generated_at="x", memo="")
...
def test_evidence_audit_rolls_up_without_new_verdict_system():
    ...
    assert report["dimension_verdicts"]["research_evidence"] == "HARD_FLAGS"
    assert report["overall_verdict"] == "HARD_FLAGS"


def test_evidence_external_check_is_information_only():
    # Evidence-audit external verification is explicitly information-only, while
    # test_needs_external_check_is_annotated above protects other skills' upstream
    # report-every-proposal behavior.
    for marker in (
        {"verdict_local": "needs_external_check"},
        {"requires_external_check": True},
    ):
        finding = _f(skill="evidence-audit", severity="critical", **marker)
        report = _report([finding])
        assert finding["_severity_final"] == "info"
        assert report["dimension_verdicts"]["research_evidence"] == "CLEAN_GIVEN_EVIDENCE"
        assert report["overall_verdict"] == "CLEAN_GIVEN_EVIDENCE"
        assert finding["_needs_external_check"] is True
        assert "evidence-audit-external-check-info-only" in finding["_adjudication"]
...
class _Args:
    def __init__(self):
        ...
        self.taxonomy_version = "0.6"
        self.paper_id = "t"
        self.generated_at = "2026-01-01T00:00:00Z"
        self.memo = ""


def _report(findings, coverage=None, run_level=2, ledger=LEDGER):
    stats = A.adjudicate(findings, run_level, ledger)
    args = _Args()
    args.observability_level = run_level
    return A.build_report(findings, args, stats,
                          anchoring_verified=ledger is not None, coverage=coverage)
...
def test_completed_and_not_applicable_do_not_gate():
    ...
    cov = {"consistency-audit": "completed", "experiment-forensics": "completed",
           ...
           "eval-design-forensics": "completed", "evidence-audit": "not_applicable"}
    r = _report([], cov)
    assert r["overall_verdict"] == "CLEAN_GIVEN_EVIDENCE"
    assert r["coverage"]["baseline-comparison-audit"] == "not_applicable"


def test_evidence_audit_reviewer_unavailable_blocks_clean_but_preserves_flags():
    coverage = {skill: "not_applicable" for skill in A.SKILL_TO_DIMENSION}

    coverage["evidence-audit"] = "completed"
    assert _report([], coverage)["overall_verdict"] == "CLEAN_GIVEN_EVIDENCE"

    coverage["evidence-audit"] = "review_unavailable"

    clean = _report([], coverage)
    assert clean["overall_verdict"] == "REVIEW_UNAVAILABLE"
    assert clean["coverage"]["evidence-audit"] == "review_unavailable"

    deterministic_flag = _f(skill="evidence-audit", severity="critical")
    flagged = _report([deterministic_flag], coverage)
    assert flagged["overall_verdict"] == "HARD_FLAGS"
    assert flagged["coverage"]["evidence-audit"] == "review_unavailable"
