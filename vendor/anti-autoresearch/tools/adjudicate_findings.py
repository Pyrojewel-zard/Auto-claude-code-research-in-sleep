...
One scoped severity rule preserves the evidence-audit contract: an
evidence-audit finding marked for external checking is information-only and
cannot raise the local verdict. Other skills retain the report-every-proposal
behavior above.
...
def adjudicate(findings, run_level, ledger_map=None):
    ...
    Everything else annotates, except for the scoped evidence-audit external-check
    rule below. Observability, false-positive risk and needs-external-check are
    fields the AUDITOR supplies about its own output —
    ...
    for f in findings:
        ...
        if f["_needs_external_check"]:
            ...
            if f.get("skill") == "evidence-audit":
                sev = "info"
                reasons.append("evidence-audit-external-check-info-only")
...
def main(argv=None):
    ...
    ap.add_argument("--taxonomy-version", default="0.6")
